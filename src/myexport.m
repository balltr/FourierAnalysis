function myexport(name,varargin)
% This function exports an eps file with larger fonts and line
% thicknesses so it can be placed in a publication.
% Called with one argument e.g. myexport('filename');
% it will convert the file to black and white
% called with two arugments e.g. myexport('filename',1);
% it will leave the figure as a color figure
...
if (color)
	exportfig(gcf,name,'FontMode','Fixed','FontSize',18,'LineMode','fixed','LineWidth',1,'Color','cmyk','FontStyle','Math');
else
	exportfig(gcf,name,'FontMode','Fixed','FontSize',18,'LineMode','fixed','LineWidth',1,'FontStyle','Math');
end

unix(['export PATH=/opt/local/bin:/opt/local/sbin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:/opt/X11/bin:/Library/TeX/texbin; epstopdf ' name '.eps']);
...
%
%   Font Parameters:
%     'FontStyle'    only allowed string is 'Math'
%          in 'Math' mode the font is set to Times and text is italicized
%          to match the latex math style
...
for k = 1:2:length(paramPairs)
  ...
  switch (param)
   ...
   case 'fontstyle'
    fontstyle = lower(value);
    if (~strcmp(fontstyle,{'math'}))
      error('FontStyle must be ''Math''.');
    end
   case 'fontsize'
...
%    old = LocalUpdateColors(allCData, 'CData', old);  % TEMPO BTH
  ...
  if (~isempty(fontstyle))
    oldfontnames = LocalGetAsCell(allText,'FontName');
    oldtext = LocalGetAsCell(allText,'String');
    newtext = ItalicText(oldtext);
	set(allText,{'String'},newtext);
    set(allText,'FontName','Times');
    % make sure we push the names
    old = LocalPushOldData(old, allText, {'FontName'}, oldfontnames);
    old = LocalPushOldData(old, allText, {'String'}, oldtext);
  end
...
function newArray = ItalicText(inArray)
n = length(inArray);
newArray = cell(n,1);
for k=1:n
  newArray{k} = ['\it ' inArray{k}];
end
