...
\section{2D Euler Equations}
	...
	************* INSERT DIAGRAM OF NODE LOCATIONS WITH N, N-1/2 AND N-1. LABEL UPSTREAM, DOWNSTREAM, AND SHOCK LOCATION  *************
	...
	$$ y_{\tau,N} = \epsilon y'_{\tau,N} = - \epsilon \mathbf{u}_u \frac{1}{\Delta x} y_{\xi,N}' $$
	
	******* INSERT SHOCK INTERFACE MOTION WAVE EQUATION FOR REFERENCE *******
	See pg 11 of A novel stabilization method for high-order shock fitting with finite element methods
	...
	\begin{equation} \label{shock_term2_pt1}
		(x_\xi (\mathbf{F}_y - y_\tau \mathbf{w}))_{N} = \frac{\Delta x}{2} \left( \mathbf{F}_{y}(\bar{\mathbf{w}}) + \epsilon \mathbf{A}_y \mathbf{w}'_N + \epsilon \mathbf{u}_u \frac{1}{\Delta x} y_{\xi,N}' (\bar{\mathbf{w}} + \epsilon \mathbf{w}'_N) \right)
	\end{equation}
	...
	\begin{equation} \label{moving_domain_equation_term3}
		...
		+ \epsilon \frac{\mathbf{u}_u \bar{\mathbf{w}}}{2} y_{\xi,N}' 
